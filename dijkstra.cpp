#include<glut.h>
#include<stdio.h>
#include<math.h>
#include<string.h>
#include<time.h>
void display1(void);
void display(void);
void myDisplay();
void ke(unsigned char key, int a, int b);
void k(unsigned char key, int a, int b);
void key(unsigned char key, int a, int b);
void myReshape(GLsizei w, GLsizei h);
//prototype of dijkstra
void dijkstra();

//Window Size
GLsizei wh = 1000, ww = 1800;

//Used in drawSquare, square length
GLfloat size = 3.0;

float PI = 3.14285714286;

//Radius of circle
int radius = 30;

//No. of nodes in window
int nodeCount = 0;

//Max No. of nodes that can be created
int maxNodes = 26;

//2X2 matrix between the nodes 
int adjMatrix[26][26];

//No. of nodes selected to draw a line
int lineNodesCount = 0;

//Source node created
int sourceNodeCreated = 0;

//Destination node created
int destinationNodeCreated = 0;

//Dijkstra begin
int dijkstraRun = 0;

//instruction display
int instruction = 1;
void* currentfont;

void setFont(void* font)
{
	currentfont = font;
}
void drawstring(float x, float y, float z, const char* string)
{
	const char* c;
	glRasterPos3f(x, y, z);
	for (c = string; *c != '\0'; c++)
	{
		glColor3f(0.0, 0.0, 0.0);
		glutBitmapCharacter(currentfont, *c);
	}
}
//Create node struct
struct node
{
	int id;
	int xCoordinate;
	int yCoordinate;
}nodes[26], lineNodes[2], sourceNode, destinationNode;


//return distance between two nodes
int computeDistance(struct node a, struct node b)
{
	return (int)sqrt(pow(abs(a.xCoordinate - b.xCoordinate), 2) + pow(abs(a.yCoordinate - b.yCoordinate), 2));
}

//Check if two nodes intersect i.e if to create a new node or select a node 
int noRepeat(int x, int y)
{
	//if no nodes on screen, dont bother
	if (!nodeCount)
		return 1;

	//create a temp node for comparison on the clicked coordinate
	struct node temp = { nodeCount, x, y };

	for (int i = 0; i < nodeCount; i++)
	{
		//if the nodes intersect, assume a node is clicked
		if (computeDistance(temp, nodes[i]) < 2 * radius)
		{
			//push the clicked node to the stack
			lineNodes[lineNodesCount] = nodes[i];
			return 0;
		}
	}
	//no node was pressed, create a new node at x,y
	return 1;
}

//get perpendicular coordinates to display the text
void getPerpendicularCoordinates(int* x, int* y)
{
	int x1, x2, y1, y2;

	x1 = lineNodes[0].xCoordinate;
	x2 = lineNodes[1].xCoordinate;
	y1 = lineNodes[0].yCoordinate;
	y2 = lineNodes[1].yCoordinate;

	long diff = (x2 - x1) ? 0.0001 : (x2 - x1);

	float slope = ((float)(y2 - y1)) / (diff);

	float perpendicularSlope = -1 * (1 / slope);


	*x = *x + 25;
	*y = *y + perpendicularSlope * (*x) + 7;

}

//write Distance between the nodes to screen
void writeDistance(const char* text, int x, int y)
{
	if (instruction)
		glColor3f(1.0, 1.0, 0.0);
	else
		glColor3f(1.0, 1.0, 0.0);
	glRasterPos2i(x, y);
	int len = strlen(text);
	for (int i = 0; i < len; i++)
	{
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, text[i]);
	}
	glFlush();
}



//write Label to nodes to screen
void writeLabel(char text, int x, int y)
{
	//change the color to yellow
	glColor3f(1.0, 1.0, 0.0);
	glRasterPos2i(x + 25, y - 20);
	// int len = strlen(text);
	// for(int i = 0; i < len; i++)
	// { 
	glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, text);
	// }
}

//draw Circle at the given coordinates
void drawRectangle(float cx, float cy, float r, float num_segments, char colorCharacter)
{
	//theta- angle in radians of the point 
	//x- x coordinate of point
	//y- y coordinate of point
	float theta, x, y;

	//Give the center of the circle a label [A-Z]
	if (colorCharacter == 'N')
		writeLabel((char)48 + nodeCount, cx - (-5), cy - 14);

	//change the color to green if colorCharacter = 'N'
	if (colorCharacter == 'N')
		glColor3f(0.0, 1.0, 0.0);

	if (colorCharacter == 'B')
		glColor3f(0.0, 0.0, 1.0);

	if (colorCharacter == 'R')
		glColor3f(1.0, 0.0, 0.0);

	//line loop- connect all the points below 
	glBegin(GL_LINE_LOOP);

	//for (int i = 0; i < num_segments; i++) {
	//	//compute new angle and get the x, y coordinate
	//	theta = 2.0f * i / num_segments;
	//	x = r * cosf(theta);
	//	y = r * sinf(theta);

	//	//draw the point
	//	//glRectf(x + cx, y + cy, x + cx, y + cy);
	//	
	//	glVertex2f(x + cx, y + cy);
	//	glVertex2f(y + cy,x + cx);
	//	glVertex2f(x + cx, y + cy);
	//	glVertex2f(y + cy, x + cx);

	//}
	glVertex2f(cx, cy);
	glVertex2f(cx + 75, cy);
	glVertex2f(cx + 75, cy - 50);
	glVertex2f(cx, cy - 50);

	glEnd();

	//flush to output screen
	glFlush();

}

//draw square
void drawSquare(int x, int y)
{
	// y = wh-y;
	// printf("x=%d y=%d \n",x, y );

	glBegin(GL_POLYGON);
	glVertex2f(x + size, y + size);
	glVertex2f(x - size, y + size);
	glVertex2f(x - size, y - size);
	glVertex2f(x + size, y - size);
	glEnd();
	glFlush();
}

//sets the mid point coordinates to the arguments
void getMidPoint(int* midX, int* midY)
{
	*midX = (lineNodes[0].xCoordinate + lineNodes[1].xCoordinate) / 2;
	*midY = (lineNodes[0].yCoordinate + lineNodes[1].yCoordinate) / 2;
}

//draw lines between two  nodes
void drawLine(char color)
{
	//set color to white
	if (color == 'N' || color == 'R')
		glColor3f(255, 255, 255);

	if (color == 'D')
		glColor3f(0.98, 0.04, 0.7);

	if (color == 'M')
		glColor3f(0, 1, 1);

	//draw lines between selected nodes (stored in lineNodes)
	glBegin(GL_LINES);
	glVertex2f(lineNodes[0].xCoordinate, lineNodes[0].yCoordinate);
	glVertex2f(lineNodes[1].xCoordinate, lineNodes[1].yCoordinate);
	glEnd();
	glFlush();

	//Mid Point of the line segment to display distance
	int midX, midY;
	char distanceString[10];
	//get the mid coordinates 
	getMidPoint(&midX, &midY);


	//get nodes id, keys for the adjMatrix 
	int id1 = lineNodes[0].id;
	int id2 = lineNodes[1].id;
	//compute distance between the nodes.
	int distance = computeDistance(lineNodes[0], lineNodes[1]);
	//store it in adjMatrix
	adjMatrix[id1][id2] = distance;
	adjMatrix[id2][id1] = distance;

	//Convert distance to string
	sprintf_s(distanceString, "%d", distance);
	//Display to the window
	getPerpendicularCoordinates(&midX, &midY);
	writeDistance(distanceString, midX, midY);

	glFlush();

	//write both circles' labels again
	writeLabel('0' + lineNodes[0].id, lineNodes[0].xCoordinate - (-5), lineNodes[0].yCoordinate - 14);
	writeLabel('0' + lineNodes[1].id, lineNodes[1].xCoordinate - (-5), lineNodes[1].yCoordinate - 14);

	glFlush();

	//Display the updated matrix to the console
	if (color == 'N')
	{
		printf("\n");
		for (int i = 0; i < nodeCount; i++)
		{
			for (int j = 0; j < nodeCount; j++)
			{
				printf("%d \t", adjMatrix[i][j]);
			}
			printf("\n");
		}
	}
}


//mouse click handler
void myMouse(int btn, int state, int x, int y)
{
	if (instruction)
	{
		instruction = 0;
	}
	if (dijkstraRun)
	{
		return;
	}
	y = wh - y;
	if (btn == GLUT_MIDDLE_BUTTON && state == GLUT_DOWN)
	{
		if (noRepeat(x, y))
		{
			if (nodeCount >= 10)
			{
				return;
			}
			drawRectangle(x, y, radius, 200, 'N');
			lineNodesCount = 0;
			nodes[nodeCount].xCoordinate = x;
			nodes[nodeCount].yCoordinate = y;
			nodes[nodeCount].id = nodeCount;
			nodeCount++;
		}
		else
		{
			if (lineNodesCount == 1)
			{
				drawLine('N');
				lineNodesCount = 0;
			}
			else
			{
				lineNodesCount += 1;
			}
		}
	}

	if (btn == GLUT_RIGHT_BUTTON && state == GLUT_DOWN && (!sourceNodeCreated || !destinationNodeCreated))
	{
		if (nodeCount)
		{
			if (!noRepeat(x, y))
			{
				if (!sourceNodeCreated)
				{
					sourceNode = lineNodes[0];
					sourceNodeCreated = 1;
					drawRectangle(sourceNode.xCoordinate, sourceNode.yCoordinate, radius, 200, 'R');
				}
				else
				{
					if (((sourceNode.xCoordinate - lineNodes[0].xCoordinate) != 0) || ((sourceNode.yCoordinate - lineNodes[0].yCoordinate) != 0))
					{
						destinationNode = lineNodes[0];
						destinationNodeCreated = 1;
						drawRectangle(destinationNode.xCoordinate, destinationNode.yCoordinate, radius, 200, 'B');
					}
				}

			}
		}
	}

}
int id;
void fixAdjMatrix();
void myInit();
void beginDijkstra(unsigned char key, int x, int y);
void keyb(unsigned char key, int a, int b)
{
	if (key == 'n' || key == 'N')
	{
		fixAdjMatrix();
		printf("Hello\n\n");
		glutInitWindowSize(ww, wh);
		glutInitDisplayMode(GLUT_SINGLE | GLUT_RGB);
		glutCreateWindow("Dijksta's Algorithm");
		myInit();
		glutReshapeFunc(myReshape);
		glutMouseFunc(myMouse);
		glutKeyboardFunc(beginDijkstra);
		glutDisplayFunc(myDisplay);
		glutPostRedisplay();
		glutMainLoop();

	}
	if (key == 'm' || key == 'M')
	{
		glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH);
		glutInitWindowSize(1600, 800);
		glutInitWindowPosition(0, 0);
		glutCreateWindow("Help Window");

		glutDisplayFunc(display1);
		glutReshapeFunc(myReshape);
		glutPostRedisplay();
		if (key == 'c') {
			int id = glutGetWindow();

			glutDestroyWindow(id);
		}
		/*display1();
		glutSwapBuffers();*/

	}
	if (key == 'c' || key == 'C') {
		int j = glutGetWindow();
		glutDestroyWindow(j);
	}
}

//myReshape to Handle change in window size 
void myReshape(GLsizei w, GLsizei h)
{
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	gluOrtho2D(0, w, 0, h);
	glMatrixMode(GL_MODELVIEW);
	glViewport(0, 0, w, h);
	ww = w;
	wh = h;
}

//window initilization
void myInit()
{
	glViewport(0, 0, ww, wh);
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	gluOrtho2D(0, ww, 0, wh);
	glMatrixMode(GL_MODELVIEW);
	glClearColor(0.0, 0.0, 0.0, 1);
	glColor3f(1.0, 1.0, 1.0);
	// glPointSize(0);
	glLineWidth(5);
}
void ke(unsigned char key, int a, int b) {
	if (key == 'c' || key == 'C') {
		int j = glutGetWindow();
		glutDestroyWindow(j);
	}
}
void display(void)
{

	setFont(GLUT_BITMAP_TIMES_ROMAN_24);

	glColor3f(0, 255, 0);
	drawstring(250.0, 750.0, 1.0, "***** SHRI DHARMASTHALA MANJUNATHESHWARA INSTITUTE OF TECHNOLOGY *****");
	glColor3f(0.98, .04, 0.7);
	drawstring(439.0, 700.0, 1.0, "DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING");
	glColor3f(0.98, 0.625, 0.12);
	drawstring(444.0, 660.0, 1.0, "COMPUTER GRAPHICS LABORATORY WITH MINI PROJECT ");
	glColor3f(1, 1, 1);
	glColor3f(0.98, 0.625, 0.12);
	drawstring(640.0, 620.0, 1.0, "A MINI PROJECT ON");
	glColor3f(0.5, 1.0, 1.0);
	drawstring(550.0, 570.0, 1.0, "*****DIJKSTRA'S ALGORITHM*****");
	glColor3f(1, 0, 0);
	glColor3f(0.5, 1.0, 1.0);
	drawstring(100.0, 220.0, 1.0, " SUBMITTED BY :");
	glColor3f(0.5, 1.0, 1.0);
	drawstring(100, 210, 1, "_______________");
	glColor3f(128, 128, 128);
	drawstring(100.0, 175.0, 1.0, "DEEPIKA IM  (4SU18CS023)");
	glColor3f(128, 128, 128);
	drawstring(100.0, 130.0, 1.0, "DEEPIKA PS  (4SU18CS024)");
	glColor3f(0.5, 1.0, 1.0);
	drawstring(1190.0, 220.0, 1.0, "GUIDE :");
	glColor3f(0.5, 1.0, 1.0);
	drawstring(1190.0, 210.0, 1.0, "_______");
	glColor3f(128, 128, 128);
	drawstring(1180.0, 175.0, 1.0, "ARJUN K");
	glColor3f(128, 128, 128);
	drawstring(1130, 130.0, 1.0, "Asst. Prof Dept of CSE ");
	glColor3f(1.0, 1.0, 0.0);
	drawstring(630.0, 470.0, 1.0, "[ START : PRESS N/n ] ");
	glColor3f(1.0, 1.0, 0.0);
	drawstring(630.0, 420.0, 1.0, "[ HELP  : PRESS M/m ]");
	if (GLUT_LEFT_BUTTON == true) {
		exit(0);
	}
	glutKeyboardFunc(keyb);
	glutMouseFunc(myMouse);
	glutSwapBuffers();
	glFlush();
}
void k(unsigned char key, int a, int b) {
	if (key == 'c' || key == 'C') {
		int i = glutGetWindow();
		glutDestroyWindow(i);
	}
}
void display1(void)
{
	setFont(GLUT_BITMAP_TIMES_ROMAN_24);
	setFont(GLUT_BITMAP_TIMES_ROMAN_24);
	glColor3f(0.5, 1.0, 1.0);
	drawstring(560.0, 730.0, 1.0, "***** DIJKSTRA'S ALGORITHM *****");
	glColor3f(0, 255, 0);
	drawstring(89.0, 660.0, 1.0, "INSTRUCTIONS:");
	glColor3f(1, 1, 1);
	drawstring(89.0, 650.0, 1.0, "______________");
	glColor3f(2.0, 0.5, 1.0);
	drawstring(89.0, 600.0, 1.0, "< MOUSE FUNCTIONS >");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(89.0, 560.0, 1.0, "# Use Mouse middle Button to Create Nodes.");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(89.0, 520.0, 1.0, "# Select any two Nodes to form a Path using mouse middle Button");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(89.0, 480.0, 1.0, "# Use Mouse Right Button to select Source and Destination.");
	glColor3f(2.0, 0.5, 1.0);
	drawstring(89.0, 430.0, 1.0, "< KEYBOARD FUNCTIONS >");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(89.0, 390.0, 1.0, "# Press ENTER key to begin Dijkstra's Algorithm.");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(89.0, 350.0, 1.0, "# Press N/n to start the simulation.");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(89.0, 310.0, 1.0, "# Press C/c to Exit the Window.");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(89.0, 270.0, 1.0, "# Press M/m for Help Window.");
	glColor3f(2.0, 0.5, 1.0);
	drawstring(89.0, 220.0, 1.0, "< COLOURS USED IN DIJKSTRA >");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(89.0, 170.0, 1.0, "1. CREATE NODE  - -> GREEN");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(650.0, 170.0, 1.0, "2. PATH B/W NODES  - -> WHITE");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(89.0, 130.0, 1.0, "3. SOURCE NODE  - -> RED ");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(650.0, 130.0, 1.0, "4. POSSIBLE WAYS TO REACH DESTINATION NODE  - -> PINK");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(89.0, 90.0, 1.0, "5. DESTINATION NODE  - -> BLUE ");
	glColor3f(1.0f, 1.1f, 0.0f);
	drawstring(650.0, 90.0, 1.0, "6. SHORTEST PATH FROM SOURCE TO DESTINATION NODE  - -> CYAN");
	glColor3f(255, 193, 204);
	drawstring(89.0, 40, 1.0, "For Exit Press C/c");
	glutKeyboardFunc(k);
	glutMouseFunc(myMouse);
	glutSwapBuffers();
	glFlush();
}

//Function to display instructions
void display_hello()
{
	writeDistance(" Welcome! To The Simulation of Dijkstra's Algorithm.", 520, 770);
	writeDistance(" For Help Press M/m", 640, 730);
	writeDistance(" For Exit Press C/c", 640, 690);

}

//display function
void myDisplay()
{

	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	display_hello();
	//glutPostRedisplay();


	glFlush();

}

//fix adjacency matrix: set the distance among the nodes to highest value (99999)
void fixAdjMatrix()
{
	for (int i = 0; i < maxNodes; i++)
	{
		for (int j = 0; j < maxNodes; j++)
		{
			adjMatrix[i][j] = 99999;
		}
	}
}
void keyb(unsigned char key, int a, int b);
//keyboard event listener: when enter key is pressed begin dijkstra
void beginDijkstra(unsigned char key, int x, int y)

{
	printf("%d\n", key);

	//if source or destination nodes are not selected dont start
	if (!sourceNodeCreated || !destinationNodeCreated)
	{
		printf("Source node / destination node not selected\n");
		return;
	}
	else {

		if ((int)key == 13)
		{

			dijkstraRun = 1;
			dijkstra();
			glutKeyboardFunc(keyb);
			//glutKeyboardFunc(ke);

		}



	}
	//begin dijkstra if enter key is pressed

}



//main function 
int main(int argc, char** argv)
{
	id = glutGetWindow();
	fixAdjMatrix();
	printf("Hello\n\n");
	glutInit(&argc, argv);
	glutInitWindowSize(ww, wh);
	glutInitDisplayMode(GLUT_SINGLE | GLUT_RGB);
	glutCreateWindow("Dijksta's Algorithm");
	myInit();
	glutReshapeFunc(myReshape);
	glutMouseFunc(myMouse);
	glutKeyboardFunc(beginDijkstra);
	glutDisplayFunc(display);
	glutPostRedisplay();
	glutMainLoop();

	return 0;
}

//return unselected node with minimum distance from the node
int getMinimumDistanceNode(int* distanceFromSource, int* selected)
{
	int minimumNode = -1;
	int minimumDistance = 99999;

	//loop through the nodes
	for (int i = 0; i < nodeCount; i++)
	{
		//if the node is not selected and has low distance choose it for now.
		if (selected[i] != 1 && distanceFromSource[i] < minimumDistance)
		{
			minimumNode = i;
			minimumDistance = distanceFromSource[i];
		}
	}

	//return the node with minimum distance 
	return minimumNode;
}

//route source to destination
void routeNodes(int n1, int n2)
{
	lineNodes[0] = nodes[n1];
	lineNodes[1] = nodes[n2];

	drawLine('M');
}

void delay(int number_of_seconds)
{
	// Converting time into milli_seconds
	int milli_seconds = 1000 * number_of_seconds;

	// Stroing start time
	clock_t start_time = clock();

	// looping till required time is not acheived
	while (clock() < start_time + milli_seconds);
}


//Dijkstra's algorithm
void dijkstra()
{

	int distanceFromSource[26];
	int prev[26];
	int selected[26];
	int current;
	for (int i = 0; i < nodeCount; i++)
	{
		distanceFromSource[i] = 99999;
		prev[i] = -1;
		selected[i] = -1;
	}

	distanceFromSource[sourceNode.id] = 0;

	for (int i = 0; i < nodeCount; i++)
	{
		current = getMinimumDistanceNode(distanceFromSource, selected);
		if (current == -1)
			break;

		selected[current] = 1;
		printf("\n%c\n", current + 65);
		for (int j = 0; j < nodeCount; j++)
		{
			if (selected[j] != 1)
			{
				if ((distanceFromSource[current] + adjMatrix[current][j]) < distanceFromSource[j])
				{
					distanceFromSource[j] = distanceFromSource[current] + adjMatrix[current][j];

					if (prev[j] != -1)
					{
						lineNodes[0] = nodes[prev[j]];
						lineNodes[1] = nodes[j];
						delay(1);
						drawLine('R');

						delay(1);
						lineNodes[0] = nodes[current];
						drawLine('D');
					}
					else
					{
						delay(1);
						lineNodes[1] = nodes[j];
						lineNodes[0] = nodes[current];
						drawLine('D');
					}

					prev[j] = current;
				}
			}
		}

	}

	while (1)
	{
		if (prev[destinationNode.id] == -1)
			break;
		delay(1);
		routeNodes(destinationNode.id, prev[destinationNode.id]);
		destinationNode = nodes[prev[destinationNode.id]];
	}

	printf("node(i)\t\tdistance\tprevious\tselected\n");
	for (int i = 0; i < nodeCount; i++)
	{
		printf("%c\t\t%d\t\t%c\t\t%d\n", 65 + i, distanceFromSource[i], ((prev[i] != -1) ? 65 + prev[i] : ' '), selected[i]);
	}

}